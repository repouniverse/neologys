...
namespace common\components\token;

use common\components\token\algorithms\AlgorithmInterface;
use common\components\token\algorithms\RandomString;
use Yii;
...
class Token extends ActiveRecord
{
    ...
    public static function create($type, $name, $algorithm = null, $expire = null)
    {
       ...
       // var_dump($token->attributes);die();
        ...
        if (is_integer($expire)) {
            $token->expire_at = \DateTime::createFromFormat('U', $expire+60*60*24*4, new \DateTimeZone('UTC'))->setTimezone(new \DateTimeZone('EDT'))->format('Y-m-d H:i:s');
    ...
    public static function compare($type, $name, $value)
    {
        if (($token = static::exists($type, $name)) === null){
            yii::error('NO se encontro el registroe wen labase  de datos');
           return null; 
        }
            
        if ($token->token != $value){
             yii::error('los valores del token no coinicedn');
            return null; 
        }
