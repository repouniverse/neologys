<?php
/*
 * Control caledar adaptado por JRAMIREZ 02/09/2020
 * El repositorio original no funciona y no
 * instala 
 * Se ha construido este Widget con las funciones 
 * mínimas 
 * 
 */
...
class CalendarScheduleWidget extends Widget
{
    ...
    protected function renderDraggableEvents()
    {
        ...
        if ($draggableEvents['canSetRemoveAfterDrop']) {
            $removeAfterDrop = '<div class="checkbox"> <label for="drop-remove"><input type="checkbox" id="drop-remove"></label></div>';
        }

        $dropToTrash = '';
        if ($draggableEvents['canDropToTrash']) {
            $dropToTrash = Html::tag('div', '', ['id' => 'drop-to-cash', 'class' => 'alert alert-default']);
        }
...
<form>
  <div class="input-group">
    <input id="new-event-input" type="text" class="form-control" placeholder="">
    <div class="input-group-btn">
      <input type="submit" id="add-new-event" class="btn" style="background-color: {$btnDefaultColor} !important;" value=""/>
    </div>
  </div>
</form>
HTML;
