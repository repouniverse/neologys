<?php
namespace common\widgets\comboajaxwidget;
...
class comboAjaxWidget extends \yii\base\Widget
{
    public $id;
    public $id_combo=null;
    public $tipo='GET';
    public $ruta='';
    ...
    {  // Register AssetBundle
        comboAjaxWidgetAsset::register($this->getView());
         if($this->isHtml){
             $this->makeJsHtml();
         }else{
              $this->makeJs();
 ...
 private function makeJsHtml(){
   $this->getView()->registerJs("$(document).ready(function() {
    $('#".$this->id_combo."').on('".$this->evento."',function(){
    var_filtro=$('#".$this->id_combo." option:selected').val();
  $.ajax({ 
   url:'".$this->ruta."',
   type:'".$this->tipo."',
   dataType:'html',
   data:{valorCombo:var_filtro},
