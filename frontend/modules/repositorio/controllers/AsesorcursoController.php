...
class AsesorcursoController extends \common\controllers\base\baseController
{
    ...
    public function actionCreate()
    {
        ...
        if ($modelalumno instanceof Alumnos) {
            if ($tienecursos = $modelalumno->hasCursosTalleres()) {
    ...
    public function actionManageFiles()
    {
        ...
        $modelalumno = Yii::$app->user->profile->persona->identidad;
        $modelVista = RepoVwAsesoresAsignados::find()->andWhere(['alumno_id' => $modelalumno->id])->all();
        ...
        //Se crean los documentos para todos los modelos.
        foreach($modelVista as $mod){
            $mod->generateDocs();
        }
        if ($modelalumno instanceof Alumnos) {
    ...
    public function actionZipear()
    {
        ...
        $ruta = $model->zipeaFiles($codocu, $offset);
