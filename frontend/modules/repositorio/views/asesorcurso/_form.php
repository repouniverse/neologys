                ...
                'columns' => [
                    ...
                    [
                        ...
                        'value' => function ($model) {
                            return $model->curso->codcur;
                        }
                    ],
                    [
                        'header' => yii::t('base_labels', 'Name course'),
                        'value' => function ($model) {
                            return $model->curso->descripcion;
                        }
                    ],
                    // 'id',
                    [
            ...
            <?= GridView::widget([
                'dataProvider' => new ActiveDataProvider([
                    'query' => common\models\masters\DocenteCursoSeccion::find()
                        ->alias('t')->select(['x.id', 't.curso_id', 't.seccion', 't.docente_id'])->distinct()->innerJoin('{{%docentes}} b', 't.docente_id=b.id')->innerJoin('{{%asesores}} x', 'x.docente_id=b.id')
                        ->andWhere([
                            'curso_id' => $cursoMatriculado->curso_id,
                            'seccion' => $cursoMatriculado->seccion,
                            'periodo' => $cursoMatriculado->periodo
                        ]),
                    ...
                    [
                        ...
                        'buttons' => [
                            'add' => function ($url, $model) use ($cursoMatriculado, $modelalumno) {
                                ...
                                $tieneAsesor = common\models\masters\AsesoresCurso::find()->alias('t')->innerJoin('{{%matricula}} b', 't.matricula_id=b.id')->andWhere([
                                        'b.curso_id' => $model->curso_id,
                                        't.asesor_id' => $model->id,
                                        'b.seccion' => $model->seccion,
                                        'b.alumno_id' => $modelalumno->id,
                                        'b.periodo' => $cursoMatriculado->periodo,
                                    ])->exists();
                                if ($tieneAsesor) {
