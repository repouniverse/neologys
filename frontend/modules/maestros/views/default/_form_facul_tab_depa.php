...
<?php Pjax::begin(['id'=>$nombreGrilla,'timeout'=>false]); ?>
    ...
    <?= GridView::widget([
        ...
        'columns' => [
             [
			    ...
			    $url= Url::to(['modal-update-depa','id'=>$model->id,'gridName'=>$nombreGrilla,'idModal'=>'buscarvalor']);
   ...
   ?>
    ...
    <?php Pjax::end(); ?>
      <p>
         <?php $url= Url::to(['modal-new-depa','id'=>$model->id,'gridName'=>$nombreGrilla,'idModal'=>'buscarvalor']);
                             //echo  Html::button(yii::t('base.verbs','Modificar Rangos'), ['href' => $url, 'title' => yii::t('sta.labels','Agregar Tutor'),'id'=>'btn_contacts', 'class' => 'botonAbre btn btn-success']); 
