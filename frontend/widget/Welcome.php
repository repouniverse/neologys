 ...
 * Theme menu widget.
 */
class Welcome extends \yii\widgets\Menu
{
    /**
     * @inheritdoc
     */
    ...
    public $submenuTemplate = "\n<ul style='color:teal;' class='dropdown-menu treeview-menu  ' {show}>\n{items}\n</ul>\n</div>\n</div>\n</div>\n";
    public $submenuTemplateDos = "\n<ul style='color:teal;' class='treeview-menu ' {show}>\n{items}\n</ul>\n";
    ...
    public function run()
    {
        ...
        if (!empty($items)) {
            ...
            yii::error(Html::tag($tag, $this->renderItems($items), $options));
    ...
    protected function renderItem($item)
    {
        if (isset($item['items'])) {
            ...
            $linkTemplate = '<div class="col-lg-4 col-md-4 col-sm-6 col-xs-6">
            <div class="info-box bg-teal-gradient">
            <a href="{url}"><span class="info-box-icon" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="color:white;">{icon}</span></a>
    ...
    protected function renderItemDos($item)
    {
        if (isset($item['items'])) {
            $labelTemplate = '<a href="{url}">{icon} {label} <span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
        ...
        foreach ($items as $i => $item) {
            ...
            if (!empty($item['items'])) {
                ...
                $menu .= strtr($this->submenuTemplate, [
                    ...
                    '{items}' => $this->renderItemsDos($item['items']),
                ]);
    ...
    protected function renderItemsDos($items)
    {
        $n = count($items);
        $lines = [];
        foreach ($items as $i => $item) {
            ...
            $menu = $this->renderItemDos($item);
            
            if (!empty($item['items'])) {
                
                $menu .= strtr($this->submenuTemplateDos, [
                    '{show}' => $item['active'] ? "style='display: block;'" : '',
    ...
    public function welcomerRenderItem($item){
